...
consome: OBS, Streamlabs, TikTok Live Studio, o que for.

### No mesmo Mac: só colar a URL

Com o Stream Kit aberto na mesma máquina, a URL do Live Studio é **idêntica à
do OBS**:
...
Copie do painel (o botão de copiar), cole no campo de link do Live Studio,
pronto. **Nenhum túnel, nenhum ngrok, nenhuma configuração.** `localhost` é a
própria máquina; o servidor já escuta exatamente aí.
...
### Quando o túnel seria necessário — e por que evitar
...
casa. Hoje o servidor escuta só em `127.0.0.1` de propósito; aceitar a rede
local pede uma opção `--host`, que ainda não existe. É meia hora de trabalho se
você precisar.

**Fora da rede:** aí o ngrok resolve, mas vale saber no que você está entrando
...
senha no painel, não abrir o túnel.
